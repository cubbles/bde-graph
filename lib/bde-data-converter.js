(function () {
  ...
  class BdeDataConverter {
     ...
     */
    convertArtifact (artifactId, manifest, baseUrl) {
      var promise = new Promise(function (resolve, reject) {
        ...
        this._fetchComponents(artifact, manifest, baseUrl).then((components) => {
   ...
   *        }
     ...
     */
    convertMember (member, manifest, baseUrl) {
      function resolveFunction (resolve, reject) {
        var componentPromise = this._fetchComponent(member, manifest, baseUrl);
     ...
     * @private
     */
    _fetchComponents (artifact, manifest, baseUrl) {
      var promises = [];
      ...
      artifact.members.forEach((member) => {
        promises.push(this._fetchComponent(member, manifest, baseUrl));
      });
     ...
     * @private
     */
    _fetchComponent (member, manifest, baseUrl) {
      var resolverFunction = function (resolve, reject) {
        var artifact;
        console.log('_fetchComponents');
