...
(function (context) {
  ...
  TheGraph.Clipboard.copy = function (graph, keys) {
    ...
    for (i = 0, len = keys.length; i < len; i++) {
      ...
      newNode.id = makeNewId(node.artifactId);  // jtrs: changed node.component to node.artifactId
  ...
  TheGraph.Clipboard.paste = function (graph, position) { // @jtrs added position parameter
    var map = {};
    ...
    for (i = 0, len = clipboardContent.nodes.length; i < len; i++) {
      ...
      // @jtrs changed position of clone or clones using positions parameter, and distance of pasted elements
      var pos = { x: meta.x, y: meta.y };
      meta.x = position.x;
      meta.y = position.y;
      if (previousPos) {
        meta.x += pos.x - previousPos.x;
        meta.y += pos.y - previousPos.y;
      }
      previousPos = pos;
      var newNode = graph.addNode(makeNewId(node.artifactId), node.artifactId, meta);
      map[ node.id ] = newNode.id;
