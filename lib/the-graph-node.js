...
(function (context) {
  ...
  TheGraph.Node = React.createFactory(React.createClass({
    ...
    componentDidMount: function () {
      ...
      if (this.props.onInportSelection) { // jtrs: added listener of select inport node
        domNode.addEventListener('tap', this.onInportSelection, true);
      }
      if (this.props.onOutportSelection) { // jtrs: added listener of select outport node
        domNode.addEventListener('tap', this.onOutportSelection, true);
      }
    ...
    onNodeSelection: function (event) {
      ...
      var toggle = (event.detail.sourceEvent.metaKey || event.detail.sourceEvent.ctrlKey || event.type.startsWith('touch'));
      this.props.onNodeSelection(this.props.nodeID, this.props.node, toggle);
    },
    /**
     * Handler method for select inport node
     * @param {Event} event touch event
     */
    onInportSelection: function (event) { // jtrs:  handler method for on inport selection
      // Don't tap app (unselect)
      event.stopPropagation();

      var toggle = (event.detail.sourceEvent.metaKey || event.detail.sourceEvent.ctrlKey || event.type.startsWith('touch'));
      this.props.onInportSelection(this.props.export.id.split('.')[1], this.props.export.metadata, toggle);
    },
    /**
     * Handler method for selct outport node
     * @param {Event} event touch event
     */
    onOutportSelection: function (event) { // jtrs:  handler method for on outport selection
      // Don't tap app (unselect)
      event.stopPropagation();

      var toggle = (event.detail.sourceEvent.metaKey || event.detail.sourceEvent.ctrlKey || event.type.startsWith('touch'));
      this.props.onOutportSelection(this.props.export.id.split('.')[1], this.props.export.metadata, toggle);
    },
    ...
    onTrack: function (event) {
      ...
      if (event.detail.state === 'start') {
        ...
        var deltaX = Math.round((event.detail.ddx || 0) / scale);
        var deltaY = Math.round((event.detail.ddy || 0) / scale);
        ...
        if (this.props.export) {
          ...
          let key = this.props.exportKey.split('.')[1];
          if (this.props.isIn) {
            this.props.graph.setInportMetadata(key, newPos);
          } else {
            this.props.graph.setOutportMetadata(key, newPos);
          }
        } else {
        ...
        if (snapToGrid) {
          if (this.props.export) {
            ...
            if (this.props.isIn) {
              this.props.graph.setInportMetadata(this.props.exportKey.split('.')[1], newPos);
            } else {
              this.props.graph.setOutportMetadata(this.props.exportKey.split('.')[1], newPos);
            }
          } else {
            this.props.graph.setNodeMetadata(this.props.nodeID, {
    ...
    showContext: function (event) {
      ...
      // Get mouse position
      var x = event.x - this.props.app.props.offsetX || 0;
      var y = event.y - this.props.app.props.offsetY || 0;
      ...
      this.props.showContext({
        ...
        itemKey: (this.props.export ? this.props.exportKey.split('.')[1] : this.props.nodeID),
        item: (this.props.export ? this.props.export : this.props.node)
      });
    },

    getContext: function (menu, options, hide) {
      var x, y, scale, appX, appY, nodeX, nodeY, deltaX, deltaY, ports, processKey, highlightPort;
      ...
      if (this.props.export) {
        return TheGraph.Menu({
          ...
          label: this.props.exportKey.split('.')[0]
        });
      }
      ...
      x = options.x;
      y = options.y;
      scale = this.props.app.state.scale;
      appX = this.props.app.state.x;
      appY = this.props.app.state.y;
      nodeX = (this.props.x + this.props.width / 2) * scale + appX;
      nodeY = (this.props.y + this.props.height / 2) * scale + appY;
      deltaX = nodeX - x;
      deltaY = nodeY - y;
      ports = this.props.ports;
      processKey = this.props.nodeID;
    ...
    render: function () {
      ...
      var keys;
      // var count;
      var processKey = this.props.nodeID || this.props.exportKey;
      ...
      var nodeOptions = {
        ...
        name: this.props.nodeID || this.props.exportKey,
        key: this.props.nodeID || this.props.exportKey,
        title: label,
  ...
  var charging = {};
  function chargeEvent (job, handler, time) { // eslint-disable-line no-unused-vars
    time = time || 500;
  ...
  function cancelEvent (job) { // eslint-disable-line no-unused-vars
    if (charging[ job ]) {
