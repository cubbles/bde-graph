...
(function (context) {
  ...
  Graph.prototype.getCoordinates = function () {
    if (!this.coordinates) {
      this.coordinates = {
        artifacts: {},
        inports: {},
    ...
    for (var i = 0; i < this.nodes.length; i++) {
      ...
      if (!this.coordinates.artifacts[this.nodes[i].id]) {
        ...
        nodeTmp.height = this.nodes[i].metadata.height;
        this.coordinates.artifacts[this.nodes[i].id] = nodeTmp;
        ...
        changes.artifacts.created[this.nodes[i].id] = JSON.parse(JSON.stringify(nodeTmp));
      } else {
        nodeTmp = this.coordinates.artifacts[this.nodes[i].id];
        var tmp;
        var changeTmp = {};
        if (nodeTmp.x !== (tmp = this.nodes[i].metadata.x + (this.nodes[i].metadata.width / 2))) {
          nodeTmp.x = changeTmp.x = tmp;
        }
        if (nodeTmp.y !== (tmp = this.nodes[i].metadata.y + (this.nodes[i].metadata.height / 2))) {
          nodeTmp.y = changeTmp.y = tmp;
        }
        if (nodeTmp.width !== (tmp = this.nodes[i].metadata.width)) {
          nodeTmp.width = changeTmp.width = tmp;
        }
    ...
    for (i in this.inports) {
      if (this.inports.hasOwnProperty(i)) {
        nodeTmp = {};
        // for squares
        nodeTmp.x = this.inports[i].metadata.x + (this.inports[i].metadata.width / 2);
        nodeTmp.y = this.inports[i].metadata.y + (this.inports[i].metadata.height / 2);
        nodeTmp.width = this.inports[i].metadata.width;
        nodeTmp.height = this.inports[i].metadata.height;
        this.coordinates.inports[this.inports[i].metadata.slotId] = nodeTmp;
      }
    }
    for (i in this.outports) {
      if (this.outports.hasOwnProperty(i)) {
        nodeTmp = {};
        // for squares
        nodeTmp.x = this.outports[i].metadata.x + (this.outports[i].metadata.width / 2);
        nodeTmp.y = this.outports[i].metadata.y + (this.outports[i].metadata.height / 2);
        nodeTmp.width = this.outports[i].metadata.width;
        nodeTmp.height = this.outports[i].metadata.height;
  ...
  Graph.prototype.setCoordinates = function (nodesMeta) {
    var nodeTmp;
    this.checkTransactionStart();
    for (var i = 0; i < nodesMeta.length; i++) {
      nodeTmp = this.getNode(nodesMeta[i].id);
      if (nodeTmp !== null) {
        nodeTmp.metadata.x = nodesMeta[i].x - (nodeTmp.metadata.width / 2);
        nodeTmp.metadata.y = nodesMeta[i].y - (nodeTmp.metadata.height / 2);
        nodeTmp.metadata.width = nodesMeta[i].width;
        nodeTmp.metadata.height = nodesMeta[i].height;
      }
    }
  ...
  Graph.prototype.getInportMetadata = function (publicPort) {
    publicPort = this.getPortName(publicPort);
    if (!this.inports[ publicPort ]) {
      return;
    }
    return this.inports[ publicPort ].metadata;
  };
  ...
  Graph.prototype.getOutportMetadata = function (publicPort) {
    publicPort = this.getPortName(publicPort);
    if (!this.outports[ publicPort ]) {
      return;
    }
    return this.outports[ publicPort ].metadata;
  };
   ...
   * @param {string} artifactId the artifactId of the component
   * @param {object} metadata the meta data of the component
   * @returns {{id: *, artifactId: *, metadata: *}|*}
   */
  Graph.prototype.addNode = function (id, artifactId, metadata) {
    var node;
    this.checkTransactionStart();
    if (!metadata) {
      metadata = {};
  ...
  Graph.prototype.addEdge = function (outNode, outPort, inNode, inPort, metadata) {
    ...
    if (!outNode && outPort.indexOf('__SLOT__') === -1) {
      outPort = '__SLOT__' + outPort;
    }
    if (!inNode && inPort.indexOf('__SLOT__') === -1) {
      inPort = '__SLOT__' + inPort;
    }
    metadata = metadata || {};
    if (!metadata.connectionId) {
      metadata.connectionId = 'con-' + Math.random().toString(36).substring(2, 7).toLowerCase();
    }
    outPort = this.getPortName(outPort);
    inPort = this.getPortName(inPort);

    edge = this.edges.find(function (edge) {
  ...
  Graph.prototype.removeEdge = function (node, port, node2, port2) {
    var edge, index, toKeep, toRemove, _i, _k, _len, _len2, _ref;
    this.checkTransactionStart();
  ...
  /**
   * Add entry to initializer list.
   * @param {object|String|Boolean|Number} data
   * @param {string} node id of the node, wich ports initializations added
   * @param {string} port this ports initializations added
   * @param {object} metadata initalization metadata
   * @returns {Array} initailisation list
   */
   ...
   * @returns {Array} initailisation list
   */
  Graph.prototype.updateInitial = function (data, node, port, metadata) {
    this.removeInitial(node, port);
    this.addInitial(data, node, port, metadata);
  };
