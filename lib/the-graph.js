...
(function (context) {
  ...
  TheGraph.mixins.Tooltip = {
    showTooltip: function (event) {
      ...
      // @jtrs: change the tooltip position -> correct with offset
      var tooltipEvent = new CustomEvent('the-graph-tooltip', {
        detail: {
          tooltip: this.props.label,
          x: event.clientX - this.props.app.state.offsetX,
          y: event.clientY - this.props.app.state.offsetY
        },
        bubbles: true
      });
  ...
  TheGraph.TextBG = React.createFactory(React.createClass({
    ...
    render: function () {
      ...
      var textAnchor = 'start'; // eslint-disable-line no-unused-vars
      var dominantBaseline = 'central'; // eslint-disable-line no-unused-vars
      var x = this.props.x;
