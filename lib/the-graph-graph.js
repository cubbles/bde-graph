  ...
  TheGraph.Graph = React.createFactory(React.createClass({
    ...
    getNodeOutport: function (graph, processName, portName, route, componentName) {
      ...
      if (!ports.outports[ portName ]) {
        let label = portName.indexOf('__SLOT__') === 0 ? portName.replace('__SLOT__', '') : portName;
    ...
    getGraphOutport: function (key) {
      ...
      if (!exp) {
        ...
        let label = key.indexOf('__SLOT__') === 0 ? key.replace('__SLOT__', '') : key;
        exp.inports[ key ] = {
          label: label,
          type: 'all',
          route: 5,
    ...
    render: function () {
      ...
      var self = this;
      var graph = this.state.graph;
      // var library = this.props.library; jtrs nevr used
      var selectedIds = [];
      ...
      var nodes = graph.nodes.map(function (node) {
        var componentInfo = self.getComponentInfo(node.arifactId);
        var key = node.id;
        if (!node.metadata) {
          node.metadata = {};
        }
        ...
        var nodeOptions = {
          ...
          sublabel: node.metadata.sublabel || node.artifactId, // jtrs: changed node.component to node.artifactId
          ...
          icon: icon,
          iconsvg: iconsvg,
          ports: self.getPorts(graph, key, node.artifactId), // jtrs: changed node.component to node.artifactId
      ...
      var edges = graph.edges.map(function (edge) {
        ...
        if (!target) {
          outport = true;
          target = graph.outports[ edge.to.port ];
        }
        ...
        // Initial ports from edgs, and give port top/last edge color
        var sourcePort = self.getNodeOutport(graph, edge.from.node, edge.from.port, route, source.artifactId);
        var targetPort = self.getNodeInport(graph, edge.to.node, edge.to.port, route, target.artifactId);

        var label, key;

        if (inport) {
          label =
            edge.from.port.replace('__SLOT__', '') +
            (edge.from.hasOwnProperty('index') ? '[' + edge.from.index + ']' : '') + ' -> ' +
            target.id + '.' +
            edge.to.port.replace('__SLOT__', '') +
            (edge.to.hasOwnProperty('index') ? '[' + edge.to.index + ']' : '');
          key = 'inport.edge.' + edge.from.port + '.' + edge.to.node + '.' + edge.to.port;
        } else if (outport) {
          label = source.id + '.' +
            edge.from.port.replace('__SLOT__', '') +
            (edge.from.hasOwnProperty('index') ? '[' + edge.from.index + ']' : '') + ' -> ' +
            edge.to.port.replace('__SLOT__', '') +
            (edge.to.hasOwnProperty('index') ? '[' + edge.to.index + ']' : '');
          key = 'outport.edge.' + edge.from.node + '.' + edge.from.port + '.' + edge.to.port;
        } else {
          label = (source.metadata.label ? source.metadata.label + '.' : '') +
            edge.from.port +
            (edge.from.hasOwnProperty('index') ? '[' + edge.from.index + ']' : '') + ' -> ' +
            (target.metadata.label ? target.metadata.label + '.' : '') +
            edge.to.port +
            (edge.to.hasOwnProperty('index') ? '[' + edge.to.index + ']' : '');
          key = edge.from.node +
            edge.from.port +
      ...
      var inports = Object.keys(graph.inports).map(function (key) {
        ...
        var metadata = inport.metadata = TheGraph.merge({
          x: 0,
          y: 0,
          width: TheGraph.config.nodeWidth,
          height: TheGraph.config.nodeHeight,
          icon: icon
        }, inport.metadata);

        var selected = (self.state.selectedNodes[ key ] === true); // jtrs: selected inports
        if (selected) {
          selectedIds.push(key);
        }
        // Node view
        var expNode = {
          ...
          graph: graph,
          node: {},
          selected: selected, // jtrs: added selected to inport node
          onInportSelection: self.props.onInportSelection, // jtrs: added onInportSelection
      ...
      var outports = Object.keys(graph.outports).map(function (key) {
        ...
        var selected = (self.state.selectedNodes[ key ] === true); // jtrs: selected outports
        if (selected) {
          selectedIds.push(key);
        }
        // Node view
        var expNode = {
          ...
          graph: graph,
          node: {},
          selected: selected, // jtrs: added selected to outport node
          onOutportSelection: self.props.onOutportSelection, // jtrs: added onOutportSelection
      ...
      if (edgePreview) {
        ...
        if (edgePreview.from) {
          ...
          if (edgePreview.from.process) {
            // Source is a node
            source = graph.getNode(edgePreview.from.process);
            sourcePort = this.getNodeOutport(graph, edgePreview.from.process, edgePreview.from.port);
            edgePreviewOptions = {
              sX: source.metadata.x + source.metadata.width,
              sY: source.metadata.y + sourcePort.y,
            ...
            // Source is a graph port
            source = graph.inports[ '__SLOT__' + edgePreview.from.port ];
            edgePreviewOptions = {
              sX: source.metadata.x + source.metadata.width,
              sY: source.metadata.y + TheGraph.config.nodeHeight / 2,
              tX: this.state.edgePreviewX,
              tY: this.state.edgePreviewY,
              route: edgePreview.metadata.route
            };
          ...
          if (edgePreview.to.process) {
            // Target is a node
            target = graph.getNode(edgePreview.to.process);
            targetPort = this.getNodeInport(graph, edgePreview.to.process, edgePreview.to.port);
            edgePreviewOptions = {
            ...
            // Target is a graph outport
            target = graph.outports[ edgePreview.to.port ];
            edgePreviewOptions = {
              sX: this.state.edgePreviewX,
              sY: this.state.edgePreviewY,
              tX: target.metadata.x,
              tY: target.metadata.y + TheGraph.config.nodeHeight / 2,
              route: edgePreview.metadata.route
            };
          }
        }
